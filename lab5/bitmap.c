...
struct pixel **read_pixel_array(FILE *image, int pixel_array_offset, int width, int height) {
  ...
  for (int i = 0; i < height; i++) {
    ...
    for (int j = 0; j < width; j++) {
      struct pixel *sp = malloc(sizeof(struct pixel));
      // Get the right pixels.
      fseek(image, pixel_array_offset + 3*j + i*width*3, SEEK_SET);
      fread(&(sp->blue), 1, 1, image);
      fread(&(sp->green), 1, 1, image);
      fread(&(sp->red), 1, 1, image);
      arr[i][j] = *sp;
      free(sp);
    }
  }
